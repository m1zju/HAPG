"""This module implements gaussian mlp Exp_paper."""
import numpy as np
...
class GaussianMLPBaseline(Baseline, Parameterized, Serializable):
    ...
    def get_vval(self, observations):
        return self._regressor.predict(observations).flatten()

    def get_vval_sym(self, obs_var, name=None, **kwargs):
        with tf.name_scope(name, "get_vval_sym", values=[obs_var]):
            vvals = L.get_output(self._output_layer, {
    ...
    def get_params(self, **tags):
        return self._regressor.get_params(**tags)
